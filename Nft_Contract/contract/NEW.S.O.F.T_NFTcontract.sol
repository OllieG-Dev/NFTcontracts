//Kongz (NFT CONTRACT  0x57a204AA1042f6E66DD7730813f4024114d74f37)
// Deployed first!!!!!
...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        // if the nft's are revealed yet or not.
        if(revealed == false) {
...
contract NFTSoftz is ERC721Namable, Ownable {
	...
	mapping(uint256 => Soft) public NFTSoftz;
	...
	// Events
	event SoftIncubated (uint256 tokenId, uint256 matron, uint256 sire);
	event SoftBorn(uint256 tokenId, uint256 genes);
	event SoftAscended(uint256 tokenId, uint256 genes);

	constructor(string memory _name, string memory _symbol, string memory _initBaseURI, string memory _initNotRevealedUri, string[] memory _names, uint256[] memory _ids) public ERC721Namable(_name, _symbol, _names, _ids) {
		_setBaseURI(_initBaseURI);
        setNotRevealedURI(_initNotRevealedUri);
		_mint(msg.sender, 1001);
		_mint(msg.sender, 1002);
		_mint(msg.sender, 1003);
		NFTSoftz[1001] = Soft(0, block.timestamp);
		NFTSoftz[1002] = Soft(0, block.timestamp);
		NFTSoftz[1003] = Soft(0, block.timestamp);
		emit SoftIncubated(1001, 0, 0);
		emit SoftIncubated(1002, 0, 0);
		emit SoftIncubated(1003, 0, 0);
		bebeCount = 3;
	}
	...
	function isValidSoft(uint256 _id) pure internal returns(bool) {
		...
		// There will only be 1000 Softz, we can fix boundaries and remove 5 ids that dont match Softz
	...
	function ascend(uint256 _tokenId, uint256 _genes, bytes calldata _sig) external {
		require(isValidSoft(_tokenId), "Not valid Soft");
	...
	function breed(uint256 _sire, uint256 _matron) external {
		...
		NFTSoftz[id] = Soft(0, block.timestamp);
		_mint(msg.sender, id);
		emit SoftIncubated(id, _matron, _sire);
	}

	function evolve(uint256 _tokenId) external {
		require(ownerOf(_tokenId) == msg.sender);
		Soft storage Soft = NFTSoftz[_tokenId];
		require(Soft.genes == 0);

		uint256 genes = breedManager.tryEvolve(_tokenId);
		Soft.genes = genes;
		emit SoftBorn(_tokenId, genes);
	}
	...
	function onERC1155Received(address _operator, address _from, uint256 _id, uint256 _value, bytes calldata _data) external returns(bytes4) {
		require(msg.sender == address(OPENSEA_STORE), "WrappedSoftz: not opensea asset");
		return NFTSoftz.onERC1155Received.selector;
	}
}