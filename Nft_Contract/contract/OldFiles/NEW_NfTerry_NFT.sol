...
contract YieldToken is ERC20("Flake", "FLAKE") {
	using SafeMath for uint256;
	...
	IFlakez public  flakezContract;

	event RewardPaid(address indexed user, uint256 reward);

	constructor(address _flakez) public{
		flakezContract = IFlakez(_flakez);
	}
	...
	function updateRewardOnMint(address _user, uint256 _amount) external {
		require(msg.sender == address(flakezContract), "Can't call this");
		uint256 time = min(block.timestamp, END);
		uint256 timerUser = lastUpdate[_user];
		if (timerUser > 0)
			rewards[_user] = rewards[_user].add(flakezContract.balanceOG(_user).mul(BASE_RATE.mul((time.sub(timerUser)))).div(86400)
	...
	function updateReward(address _from, address _to, uint256 _tokenId) external {
		require(msg.sender == address(flakezContract));
		if (_tokenId < 1001) {
			uint256 time = min(block.timestamp, END);
			uint256 timerFrom = lastUpdate[_from];
			if (timerFrom > 0)
				rewards[_from] += flakezContract.balanceOG(_from).mul(BASE_RATE.mul((time.sub(timerFrom)))).div(86400);
			if (timerFrom != END)
				lastUpdate[_from] = time;
			if (_to != address(0)) {
				uint256 timerTo = lastUpdate[_to];
				if (timerTo > 0)
					rewards[_to] += flakezContract.balanceOG(_to).mul(BASE_RATE.mul((time.sub(timerTo)))).div(86400);
				if (timerTo != END)
					lastUpdate[_to] = time;
			}
		}
	...
	function getTotalClaimable(address _user) external view returns(uint256) {
		uint256 time = min(block.timestamp, END);
		uint256 pending = flakezContract.balanceOG(_user).mul(BASE_RATE.mul((time.sub(lastUpdate[_user])))).div(86400);
...
contract NFTerryz is ERC721Namable, Ownable {
   ...
   string public notRevealedUri;
	...
	mapping(uint256 => nfterry) public nfterryz;
	...
	//// Events
	//event NFterryIncubated (uint256 tokenId, uint256 matron, uint256 sire);
	//event NFterryBorn(uint256 tokenId, uint256 genes);
	//event NFterryAscended(uint256 tokenId, uint256 genes);

    constructor(string memory _name, string memory _symbol, string memory _initBaseURI, string memory _initNotRevealedUri) public ERC721Namable(_name, _symbol, _initBaseURI, _initNotRevealedUri) {
    ...
    returns (string memory)
  {
	...
	function onERC1155Received(address _operator, address _from, uint256 _id, uint256 _value, bytes calldata _data) external returns(bytes4) {
		require(msg.sender == address(OPENSEA_STORE), "WrappedNFTerryz: not opensea asset");
		return NFTerryz.onERC1155Received.selector;
	}
}