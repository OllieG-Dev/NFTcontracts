...
contract YieldToken is ERC20("Flake", "FLAKES") {
	using SafeMath for uint256;
	...
	INFTerryz public  nfterryzContract;

	event RewardPaid(address indexed user, uint256 reward);

	constructor(address _nfterryz) public{
		nfterryzContract = INFTerryz(_nfterryz);
	}
...
contract NFTerry is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  constructor(
    ...
    setNotRevealedURI(_initNotRevealedUri);
		...
		NFterryz[1001] = NFterry(0, block.timestamp);
		NFterryz[1002] = NFterry(0, block.timestamp);
		NFterryz[1003] = NFterry(0, block.timestamp);
		emit NfterryIncubated(1001, 0, 0);
		emit NfterryIncubated(1002, 0, 0);
		emit NfterryIncubated(1003, 0, 0);
		bebeCount = 3;
  }
    ...
    returns (string memory)
  {
	...
	function ascend(uint256 _tokenId, uint256 _genes, bytes calldata _sig) external {
		require(isValidNFterry(_tokenId), "Not valid NFterry");
	...
	function breed(uint256 _sire, uint256 _matron) external {
		...
		NFterryz[id] = NFterry(0, block.timestamp);
		_mint(msg.sender, id);
		emit NfterryIncubated(id, _matron, _sire);
	}
  ...
  function evolve(uint256 _tokenId) external {
		...
		NFterry storage NFterry = NFTerryz[_tokenId];
		require(NFterry.genes == 0);

		uint256 genes = breedManager.tryEvolve(_tokenId);
		kong.genes = genes;
		emit NfterryBorn(_tokenId, genes);
  }
